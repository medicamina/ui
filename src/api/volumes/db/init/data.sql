...
-- genders
CREATE TABLE publc.genders as enum (
...
ALTER TABLE public.genders OWNER TO supabase_admin;

CREATE POLICY "Select only for all users"
...
ALTER TABLE public.user_details OWNER TO supabase_admin;

COMMENT ON TABLE public.user_details IS 'Personal details of user';
...
ALTER TABLE ONLY public.user_details
  ADD CONSTRAINT user_details_country_iso2_fkey FOREIGN KEY (country) REFERENCES public.countries(iso2);

CREATE POLICY "Enable insert for self user only" 
  ON public.user_details FOR INSERT TO authenticated WITH CHECK ((auth.uid() = user_id));

CREATE POLICY "Enable select for authenticated users only"
  ON public.user_details FOR SELECT TO authenticated WITH CHECK (true);
